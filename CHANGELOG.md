...
- 兼容本次 Addressables 对六个小图标 Bundle 的路径重命名。部署合同支持按实际存在文件从新旧候选路径中选择，旧 build 与旧皮肤包无需因路径变化失效。
- 在游戏保持运行期间完成只读日志分析与离线 Bundle 保存/回读验证；24 个相关 Bundle、33 个 Texture2D 写入均通过结构、尺寸、BC7 与回读检查。未停止游戏，也未修改正在运行的游戏目录。
- 英雄 FMOD bank 未变化，现有精确语音 GUID/selector 路由继续适用；当前运行日志同时确认头像自测、对手头像保护与外部语音路由仍在新客户端生效。

## 1.5.4

- 部署页的职业和被替换皮肤下拉框现在只展示当前软件实际随附适配器的目标；游戏目录中检测到、但尚未兼容的皮肤不会进入可选项。
- 已保存的旧映射如果指向未兼容或已经消失的皮肤，会回到未完成状态并要求重新选择，不能绕过下拉框直接部署。
- 目录中的 `supported` 文本不再单独作为兼容证据；必须能够解析到随软件发布的适配器。已知适配器在新游戏 build 下仍可标记为“兼容模式”，并继续接受部署前结构校验。

## 1.5.3

- 修复部署页把“已知适配器的兼容模式”误判为“尚无已验证适配器”并直接拒绝部署的问题；部署能力现在以软件随附的适配器为准，不再以可能过期的目录状态作为硬开关。
- Vanessa 等已知英雄在游戏 build 或目录状态变化后会显示“兼容部署”，并在真正写入前按当前 Bundle、Texture2D 与保存回读结果做结构校验；结构不兼容时安全停止，不会先修改游戏。
- 区分“映射目标已经从当前目录消失”和“确实没有适配器”两类错误，避免 v1.4.11 中所有异常状态都显示同一句“尚无已验证适配器”。

## 1.5.2

- 逐槽位模式的“角色立绘”右侧不再显示动态资源无位图的空提示；现在从当前英雄的游戏原生全身图生成参考，并放入立绘槽位相同的 1086×1448 画布、目标边界、中心线和底边锚点中，便于直接比较位置与大小。
- 参考图继续优先读取 Manager 保存的已验证原版备份，未部署时读取当前游戏 Bundle；不会把已经替换的皮肤误当成原版。
- 不采用原生 Spine setup pose 作为参考，因为该姿态尚未应用游戏动画时间轴，会出现部件翻转或分离；静态全身图提供稳定且完整的视觉基准。

## 1.5.1

- Skin Manager 的外部替换语音现在实时跟随游戏内“主音量”和“语音音量”；菜单语音同样归入语音音量，运行时无法绑定这些精确控制项时保留原生 FMOD 播放。
- 明确游戏及模组的验收启动路径必须经过 Steam；直接运行 `TheBazaar.exe` 缺少 Steam 会话，不作为可用启动方式或完整兼容性证据。
...
- 已知英雄适配器不再因为新的 Steam build ID 或 Bundle 哈希整体阻断部署。管理器会直接验证实际 Bundle 路径、Texture2D 名称、尺寸、保存回读和目录 CRC；验证成功即继续，结构不兼容则跳过原生槽位并保留运行时安全的替换。
- Steam 或其他工具改动部分托管文件后，取消部署与重新部署采用逐项恢复：能确认来源的文件恢复，外部改动文件与相关备份保留，并生成恢复报告，不再要求用户先验证游戏文件才能继续使用管理器。
- 通过管理器启动游戏时自动尝试修复“新目录 + 旧补丁 Bundle”的已知 CRC 状态；修复失败只记录警告，不阻止 Steam 启动游戏。
- 软件更新仍只在启动时检查并询问用户；本版本发布后刻意不替用户安装，用于从旧版实测完整更新交互。
...
- 校验改为同时约束平均误差、高误差通道占比和严重误差通道占比；允许 BC7 正常的稀疏离群点，但仍拒绝大面积颜色偏差或编码损坏。

## 1.4.11

- 修复 v1.4.10 在 Steam 已更新旧英雄原生 Bundle、但本次部署选择的是另一个英雄时，仍使用旧部署皮肤包内的过期哈希合同判断旧事务，导致安全退休未触发、再次落入 `Cannot safely cancel deployment` 的问题。
- 旧事务退休现在以当前管理器随附、且明确支持当前 Steam build 的适配器为权威；仍保留完整事务、官方哈希白名单与混合状态 fail-closed 约束。

## 1.4.10

- 修复 Steam 更新或“验证游戏文件”已经把整组原生 Bundle 与 Addressables 目录重置为受当前适配器认可的新原版后，旧部署事务仍阻止重新部署的问题；重部署会在确认所有旧补丁均已消失、每个 Bundle 都是白名单原版哈希且目录也不再是旧补丁版本后，保留新游戏文件并安全退休旧备份与授权记录。
- 混合状态继续严格拒绝：只要仍有一个旧补丁 Bundle、旧补丁目录、未知 Bundle 哈希或缺失文件，就不会清理备份和安装记录，避免把新旧 Addressables 状态拼成不可启动的游戏。
- 修复 Vanessa 等英雄的多个逻辑槽位实际共用同一个原生 Texture2D 时，逐槽位不同图片导致 `conflicting native patch images` 的部署失败；物化部署包现在按适配器顺序选择物理纹理的规范槽位，并让商店/市场、收藏/每日等原生别名共享同一张部署图片。

## 1.4.9

- 修复空白或源文件已经失效的逐槽位脏标记仍被提交为透明图片、从而遮蔽默认模式已生成槽位的问题；默认预览与发布现在只合并具有可读取素材源的有效覆盖。
- 已生成的商店/对局头像不会再因为无效逐槽位覆盖而显示棋盘格；检查确认工作区中的 `portrait_gameplay.png` 与独立背景仍完整，问题只发生在内存覆盖选择层，素材没有丢失。
- 透明 PNG 仍可作为用户明确提供的有效覆盖；只有没有素材或素材路径失效的覆盖会回退到自动生成底稿，并同步修正覆盖数量与发布分流判断。

## 1.4.8

- 修复逐槽位调整把 `assets/*.png` 这种可变生成结果继续当作编辑源的问题；第一次修改槽位时会把涉及的成品、背景和人物层快照到独立的 `authoring/manual_drafts`，后续生成不会覆盖该源图。
- 同一个位置偏移或缩放参数现在可以反复编辑、导入和导出而保持幂等，不会在第二次编辑时把已缩放的结果再次缩放，造成比例平方或位移累计。
- 增加“自动模式覆盖生成文件后连续两次物化”回归测试，同时校验输出像素、透明边界和源图颜色保持一致。

## 1.4.7

- 皮肤制作的两个模式改为“默认底稿 + 稀疏逐槽位覆盖”的单一草稿模型；切换选项卡只提交内存状态，不再同步复制素材、编码全部 PNG、重建工作区或打包，因此不会闪回原选项卡，也不再阻塞 UI 十几秒。
- 默认底稿发生变化时，逐槽位模式保持可见并在后台更新未覆盖槽位；已经手工调整或清空的槽位继续保留。切回默认模式后，五个生成预览会立即优先显示逐槽位覆盖，并明确标出覆盖数量。
- 默认模式中的“导入到皮肤库”和“导出到指定位置”会自动合并逐槽位覆盖；逐槽位模式把相同的导入、ZIP 导出操作提升到顶部工具栏，生成过程移到后台线程。
- 逐槽位落盘改为批量替换图像并只保存一次工作区状态，移除每个槽位反复清空、保存和导入造成的额外 I/O；增加模式切换无磁盘构建和覆盖预览回归测试。
